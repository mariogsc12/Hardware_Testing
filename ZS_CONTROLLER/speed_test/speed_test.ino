...
int POT_PIN_IN = A0;                  // Analog potentiomer for speed

volatile float feedback=0.00, tic, tac;                                 // interrupt function variables
float  pot=0, _speed =0, target, trgt_min=0, trgt_max=255, fb_min=0, fb_max=0;     // target_PWM value - min and max values between target_PWM and feedback
...
void loop() {
  ...
  _speed = feedback*4;
  Serial.print("\n  target: ");Serial.print(target,0); Serial.print("  speed: ");Serial.println(_speed,0); delay(100);
}
